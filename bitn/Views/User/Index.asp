...
rs_lista_cmd.CommandText = "SELECT * FROM tbl_usuarios WHERE Habil = ? ORDER BY NomeCompleto ASC" 
    ...
    <div class="container">
      <div class="row">
        ...
        <div class="col-lg-9 order-1 order-lg-2">
              ...
              <!-- share box start -->
          <div class="card"> 
                <h4 class="widget-title">Gerenciar Usuários</h4>
...
<table border="1" class="table table-bordered table-borderless table-hover table-striped table-responsive" style="width:100%;">
  ...
  <% While ((Repeat1__numRows <> 0) AND (NOT rs_lista.EOF)) %>
    <tr style="font-size:12px;">
      <td><strong><%=(rs_lista.Fields.Item("codUsuario").Value)%></strong></td>
      <td><strong><%=(rs_lista.Fields.Item("Usuario").Value)%></strong><div style="font-size:10px;"><%=(rs_lista.Fields.Item("Tipo").Value)%></div></td>
      <td><strong><%=(rs_lista.Fields.Item("NomeCompleto").Value)%></strong>
      <div style="font-size:10px;"><%=(rs_lista.Fields.Item("DataNascimento").Value)%></div>
	  <div style="font-size:10px;"><%=(rs_lista.Fields.Item("Profissao").Value)%></div>
      </td>
      <td>
	  <%=(rs_lista.Fields.Item("Email").Value)%>
      <div style="font-size:10px;"><%=(rs_lista.Fields.Item("Cidade").Value)%> / <%=(rs_lista.Fields.Item("Estado").Value)%></div></td>
      <td>
	  <div style="font-size:10px;">Criado: <%=(rs_lista.Fields.Item("CriadoEm").Value)%></div>
      <div style="font-size:10px;">Alterado: <%=(rs_lista.Fields.Item("AlteradoEm").Value)%></div>
      <div style="font-size:10px;">Último: <%=(rs_lista.Fields.Item("UltimoPost").Value)%></div>
      </td>
      ...
      <div><strong>Amigos:</strong> <%Amigos=(rs_lista.Fields.Item("Amigos").Value)%><%if isNull(Amigos) or trim(Amigos) = "" then%>0<%else%><%=Amigos%><%end if%></div>
      </td>
      <td style="text-align:center;"><button class="edit-btn" onClick="fnEditPerfilAdm(<%=(rs_lista.Fields.Item("codUsuario").Value)%>)">Editar</button></td>
    </tr>
    <% 
