...
SQL = "select * from tbl_midia_comentarios_likes Where codUsuario = "&rs_usuario("codUsuario")&" and Habil = 1 and codComentario = "&request.QueryString("codComentario")
set rs = DbConn.Execute(sql)

'Se tiver tira.
if not rs.eof and not rs.bof then
	SQL = "update tbl_midia_comentarios_likes set Habil = 0 Where codUsuario = "&rs_usuario("codUsuario")&" and codComentario = "&request.QueryString("codComentario")
	corLike = "#CCC"
else
'Se nao adiciona
	SQL = "insert into tbl_midia_comentarios_likes (codUsuario, codMidia, codComentario, Habil) values ("&rs_usuario("codUsuario")&", "&request.QueryString("codMidia")&", "&request.QueryString("codComentario")&",1)"
	corLike = "#069"
end if
set rs = DbConn.Execute(sql)

'Conta a quantidade de Likes
	SQL = "select count(codComentario) as Likes from tbl_midia_comentarios_likes Where codComentario= "&request.QueryString("codComentario")&" and Habil = 1"
	set rs = DbConn.Execute(sql)

	Likes = rs("Likes")

'Atualiza o artigo
	SQL = "update tbl_midia_comentarios set Likes = "&Likes&" Where codComentario= "&request.QueryString("codComentario")&""
	set rs = DbConn.Execute(sql)
