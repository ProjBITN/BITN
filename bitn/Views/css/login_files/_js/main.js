...
function fnPesquisar(Valor){
	carregaAjax('divPesquisa','/Views/User/Pesquisar.asp?Valor='+Valor)
}
function fnPerfil(codUsuario, NomeCompleto, Foto, Cidade, Estado, Profissao){
	document.getElementById('lblNomeCompleto').href="/Perfil/"+codUsuario+'/';
	document.getElementById('lblNomeCompleto2').href="/Perfil/"+codUsuario+'/';
	document.getElementById('lblNomeCompleto').innerHTML = NomeCompleto;
	document.getElementById('lblFoto').src = Foto;
	document.getElementById('lblTexto').innerHTML = Cidade+" - "+Estado+"<br />Profissão: "+Profissao;
	document.getElementById('lblProfissao').innerHTML = Profissao;
	document.getElementById('lblcodPerfil').value = codUsuario;
}
function fnAmigo2(){
	fnAmigo(document.getElementById('lblcodPerfil').value);
}

function fnVincularEdit(valor){
	document.getElementById('VincularEdit').disabled = true;
		if(valor==3){
			document.getElementById('VincularEdit').disabled = false;
		}
}

function fnCurtir(codMidia){
	carregaAjax('divLike'+codMidia,'/Views/post/like.asp?codMidia='+codMidia)
}

function fnCurtirCom(codMidia, codComentario){
	carregaAjax('divLikeCom'+codComentario,'/Views/post/likeCom.asp?codMidia='+codMidia+'&codComentario='+codComentario)
}
